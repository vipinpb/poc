 ...
 * <p>
 * Normal users should use {@link Gauge}, {@link Counter}, {@link Summary} and
 * {@link Histogram}.
 * <p>
 * Subclasssing Collector is for advanced uses, such as proxying metrics from
 * another monitoring system. It is it the responsibility of subclasses to
 * ensure they produce valid metrics.
 * 
 * @see <a
 *      href="http://prometheus.io/docs/instrumenting/exposition_formats/">Exposition
 *      formats</a>.
 */
public abstract class Collector {
	...
	static public class MetricFamilySamples {
		...
		public MetricFamilySamples(String name, String unit, Type type,
				String help, List<Sample> samples) {
			if (!unit.isEmpty() && !name.endsWith("_" + unit)) {
				throw new IllegalArgumentException(
						"Metric's unit is not the suffix of the metric name: "
								+ name);
			}
			if ((type == Type.INFO || type == Type.STATE_SET)
					&& !unit.isEmpty()) {
				throw new IllegalArgumentException(
			...
			if (type == Type.COUNTER) {
				...
				for (Sample s : samples) {
					...
					mungedSamples.add(new Sample(n, s.labelNames,
							s.labelValues, s.value, s.timestampMs));
				}
			}
			this.name = name;
			this.unit = unit;
		...
		public String toString() {
			return "Name: " + name + " Unit:" + unit + " Type: " + type
					+ " Help: " + help + " Samples: " + samples;
		}
		...
		public static class Sample {
			...
			public final Long timestampMs; // It's an epoch format with
											// milliseconds value included (this
			...
			public boolean equals(Object obj) {
				...
				return other.name.equals(name)
						...
						&& other.value == value
						&& ((timestampMs == null && other.timestampMs == null) || (other.timestampMs != null)
			...
			@Override
			public String toString() {
				return "Name: " + name + " LabelNames: " + labelNames
						+ " labelValues: " + labelValues + " Value: " + value
	...
	public interface Describable {
		/**
		 * Provide a list of metric families this Collector is expected to
		 * return.
		 *
		 ...
		 * with auto describe enabled (which is the case for the default
		 * registry) then {@link collect} will be called at registration time
		 * instead of describe. If this could cause problems, either implement a
		 * proper describe, or if that's not practical have describe return an
		 * empty list.
		 */
